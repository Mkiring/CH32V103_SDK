...
#ifndef __CH103UFI_H__
#define __CH103UFI_H__

#define CH103_LIB_VER		0x10
...
#define ERR_CH103_ERROR			0x81	/* CH103硬件错误,可能需要复位CH103 */
...
#endif
/* 代码2XH-3XH用于USB主机方式的通讯失败代码,由CH103子程序模仿CH375的返回 */
/* 代码1XH用于USB主机方式的操作状态代码,由CH103子程序模仿CH375的返回 */
...
//	} Open;								/* CMD_FileOpen, 枚举文件, CH103vFileSize最高位为1则各调用xFileNameEnumer,为0则返回指定序号的文件名 */
	struct {
...
/* 子程序库中提供的变量 */
extern	UINT8V	CH103IntStatus;				/* CH103操作的中断状态 */
extern	UINT8V	CH103DiskStatus;			/* 磁盘及文件状态 */
extern	UINT8	CH103vDiskFat;				/* 逻辑盘的FAT标志:1=FAT12,2=FAT16,3=FAT32 */
extern	UINT8	CH103vSecPerClus;			/* 逻辑盘的每簇扇区数 */
extern	UINT8	CH103vSectorSizeB;			/* log2(CH103vSectorSize) */
extern	UINT32	CH103vStartLba;				/* 逻辑盘的起始绝对扇区号LBA */
extern	UINT32	CH103vDiskRoot;				/* 对于FAT16盘为根目录占用扇区数,对于FAT32盘为根目录起始簇号 */
extern	UINT32	CH103vDataStart;			/* 逻辑盘的数据区域的起始LBA */
extern	UINT32	CH103vStartCluster;			/* 当前文件或者目录的起始簇号 */
extern	UINT32	CH103vFileSize;				/* 当前文件的长度 */
extern	UINT32	CH103vCurrentOffset;		/* 当前文件指针,当前读写位置的字节偏移 */
extern	UINT32	CH103vFdtLba;				/* 当前FDT所在的LBA地址 */
extern	UINT32	CH103vLbaCurrent;			/* 当前读写的磁盘起始LBA地址 */
extern	UINT16	CH103vFdtOffset;			/* 当前FDT在扇区内的偏移地址 */
extern	UINT16	CH103vSectorSize;			/* 磁盘的扇区大小 */
extern	UINT8	CH103vCurrentLun;			/* 磁盘当前操作逻辑单元号 */
extern	BOOL	CH103vSubClassIs6;			/* USB存储类设备的子类为6,0则非6 */
extern	PUINT8	pDISK_BASE_BUF;		/* 指向外部RAM的磁盘数据缓冲区,缓冲区长度不小于CH103vSectorSize,由应用程序初始化 */
extern	PUINT8	pDISK_FAT_BUF;		/* 指向外部RAM的磁盘FAT数据缓冲区,缓冲区长度不小于CH103vSectorSize,由应用程序初始化 */

extern	CMD_PARAM_I	mCmdParam;				/* 命令参数 */

extern	UINT8	RxBuffer[ MAX_PACKET_SIZE ];  // IN, must even address
//#define	RxBuffer	pHOST_RX_RAM_Addr
extern	UINT8	TxBuffer[ MAX_PACKET_SIZE ];  // OUT, must even address
//#define	TxBuffer	pHOST_TX_RAM_Addr
...
#endif
extern	UINT8	CH103ReadSector( UINT8 SectCount, PUINT8 DataBuf );	/* 从磁盘读取多个扇区的数据到缓冲区中 */
#ifdef	EN_DISK_WRITE
extern	UINT8	CH103WriteSector( UINT8 SectCount, PUINT8 DataBuf );	/* 将缓冲区中的多个扇区的数据块写入磁盘 */
#endif

extern	UINT8	CH103DiskConnect( void );	/* 检查磁盘是否连接并更新磁盘状态 */
extern	void	xFileNameEnumer( void );	/* 调用外部定义的子程序,文件名枚举回调子程序 */

extern	UINT8	CH103LibInit( void );		/* 初始化CH103程序库,操作成功返回0 */

/* 子程序库中提供的子程序 */
/* 下述子程序中, 文件操作子程序CH103File*和磁盘查询子程序CH103DiskQuery都可能会用到磁盘数据缓冲区pDISK_BASE_BUF,
   并且有可能在pDISK_BASE_BUF中保存了磁盘信息, 所以必须保证pDISK_BASE_BUF不被用于其它用途,
   如果RAM较少, 要将pDISK_BASE_BUF临时用于其它用途, 那么在临时用完后必须调用CH103DirtyBuffer清除磁盘缓冲区 */
extern	UINT8	CH103GetVer( void );		/* 获取当前子程序库的版本号 */
extern	void	CH103DirtyBuffer( void );	/* 清除磁盘缓冲区 */
extern	UINT8	CH103BulkOnlyCmd( PUINT8 DataBuf );	/* 执行基于BulkOnly协议的命令 */
extern	UINT8	CH103DiskReady( void );		/* 查询磁盘是否准备好 */
extern	UINT8	CH103AnalyzeError( UINT8 iMode );	/* USB操作失败分析CH103IntStatus返回错误状态 */
extern	UINT8	CH103FileOpen( void );		/* 打开文件或者枚举文件 */
extern	UINT8	CH103FileClose( void );		/* 关闭当前文件 */
#ifdef	EN_DISK_WRITE
extern	UINT8	CH103FileErase( void );		/* 删除文件并关闭 */
extern	UINT8	CH103FileCreate( void );	/* 新建文件并打开,如果文件已经存在则先删除后再新建 */
extern	UINT8	CH103FileAlloc( void );		/* 根据文件长度调整为文件分配的磁盘空间 */
#endif
extern	UINT8	CH103FileModify( void );	/* 查询或者修改当前文件的信息 */
extern	UINT8	CH103FileQuery( void );		/* 查询当前文件的信息 */
extern	UINT8	CH103FileLocate( void );	/* 移动当前文件指针 */
extern	UINT8	CH103FileRead( void );		/* 从当前文件读取数据到指定缓冲区 */
#ifdef	EN_DISK_WRITE
extern	UINT8	CH103FileWrite( void );		/* 向当前文件写入指定缓冲区的数据 */
#endif
extern	UINT8	CH103ByteLocate( void );	/* 以字节为单位移动当前文件指针 */
extern	UINT8	CH103ByteRead( void );		/* 以字节为单位从当前位置读取数据块 */
#ifdef	EN_DISK_WRITE
extern	UINT8	CH103ByteWrite( void );		/* 以字节为单位向当前位置写入数据块 */
#endif
extern	UINT8	CH103DiskQuery( void );		/* 查询磁盘信息 */
extern	void	CH103SaveVariable( void );	/* 备份/保存/恢复子程序库的变量,用于子程序库在多个芯片或者U盘之间进行切换 */

//extern	void	Delay_Us( UINT32 n );		// 以uS为单位延时
//extern	void	Delay_Ms( UINT32 n );		// 以mS为单位延时
extern	UINT8	USBHostTransact( UINT8 endp_pid, UINT8 tog, UINT32 timeout );	// CH103传输事务,输入目的端点地址/PID令牌,同步标志,NAK重试时间,返回0成功,超时/出错重试
...
extern	void	CopySetupReqPkg( const UINT8 * pReqPkt );  // 复制控制传输的请求包
//extern	UINT8	CtrlGetDeviceDescr( PUINT8 DataBuf );  // 获取设备描述符
extern	UINT8	CtrlGetConfigDescr( PUINT8 DataBuf );  // 获取配置描述符
...
//extern	UINT8	CtrlGetHubDescr( PUINT8 DataBuf );  // 获取HUB描述符
//extern	UINT8	HubGetPortStatus( UINT8 HubPortIndex, PUINT8 DataBuf );  // 查询HUB端口状态
